...
def onedim_onecomp_step(cprev, v, Kp, Kn, Jp, Jn, dt):
    ...
    Parameters:
        ...
        - Jp (numpy.ndarray float64):  
            - Positive direction influx (mol/s) of concentration at the system's left boundary. Length = len(tsamp) (ground truth) or len(tsamp) - 1 (inversion).  
        - Jn (numpy.ndarray float64):  
            - Negative direction influx (mol/s) of concentration at the system's right boundary. Length = len(tsamp) (ground truth) or len(tsamp) - 1 (inversion).  
        - dt (float64):  
    ...
    cupdate[0] = cprev[0] + (dt/v[0]) * (Jp + Jn_interface[1] - Jp_interface[0] - Jn_interface[0])
    
    cupdate[1:-1]=cprev[1:-1] + (dt/v[1:-1])*( Jp_interface[:-2] - Jn_interface[1:-1] - Jp_interface[1:-1] + Jn_interface[2:])

    cupdate[-1] = cprev[-1] + (dt/v[-1]) * (Jn + Jp_interface[-2] - Jn_interface[-1] - Jp_interface[-1])
...
def onedim_twocomp_flow_diff(Lx, dx, tsamp, dtsim, fa, fv, F, Da, Dv, va, vv, Jpa, Jpv, Jna, Jnv):
    ...
    '''
    from tkspace.forward_models.onedim.fm_utilities import interp_linear as interpolate
    from tkspace.forward_models.onedim.flow import onedim_twocomp_step
    from numpy import max as npmax
