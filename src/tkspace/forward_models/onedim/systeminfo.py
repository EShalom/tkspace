...
def geometry():
    ...
    max_vals = 14, 70, 4.5e-4, 4.5e-3, 0.07, 0.7, 0.8 * (Lx*Ly*Lz) * 1e-3#fmax3.05, umax, Dmax, psDmax, Fmax, Kvamax, Jamax, Jvmax
...
def geometry_cm():
    ...
    Lx, Ly, Lz = 25.6, 0.8, 0.9 # Specfiy system length parameters (cm)
    dx, dy, dz = 0.01, 0.8, 0.9 # Specify forward model ground truth model spatial resolution (cm)
    Dx, Dy, Dz = 0.8, Ly, Lz # Specify Downsampling 'measurement' spatial resolution (cm)

    T   =  80# 0.0001 Sepcify temporal reso;utions for minimum cfl time, sampling time and total system evolution time
    
    max_vals = 1.4, 20, 4.5e-4, 4.5e-3, 0.15,20, 0.8 * (Lx*Ly*Lz) * 1e-3#fmax3.05, umax, Dmax, psDmax, Fmax, Kvamax, Jamax, Jvmax
