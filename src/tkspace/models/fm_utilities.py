...
def fmodsetup_1d1c(fmod, Pnorm, Pmax, dtsim, res, sysdim, tsamp, pic, inv):
    ...
    if pic == 1:
        f, D, v, Jp, Jn = param_extract_flow_1d1c(P, Nx)
        Jp *= H(f,0)
        Jn *= H(-f,0)
    elif pic == 2:
...
def fmodsetup_1d2c(fmod, Pnorm, Pmax, dtsim, res, sysdim, tsamp, pic, inv):
    ...
    from numpy import heaviside as H
    from numpy import append
    from TKfunctions.forward_models.onedim.fm_utilities import parker2006AIF_flexible as aif
    dx, dy, dz = res
    dxyz = dx*dy*dz
    Lx, Ly, Lz, T = sysdim
...
def Ftofafv_1d2c(F,fa1,fv1,dx):
    ...
    Parameters:
        ...
        - fa1 (float64):  
            - Left boundary value of the arterial flow. (mL/s/mm^2).  
        - fv1 (float64):  
            - Left boundary value of the venous flow. (mL/s/mm^2).  
    
    Returns:
        - fa (numpy.ndarray float64):  
            - Arterial flow at interfaces. (mL/s/mm^2).          
        - fv (numpy.ndarray float64):  
            - Venous flow at interfaces. (mL/s/mm^2).  
    '''
