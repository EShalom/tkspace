...
.. math:: \\frac{\\partial C^{a}}{\\partial t}=-\\vec{\\nabla} \\cdot \\vec{u}^{a} C^{a}+\\vec{\\nabla} \\cdot D^{\\star a} \\vec{\\nabla} C^{a}-K^{va}C^{a}

.. math:: \\frac{\\partial C^{v}}{\\partial t}=-\\vec{\\nabla} \\cdot \\vec{u}^{v} C^{a}+\\vec{\\nabla} \\cdot D^{\\star v} \\vec{\\nabla} C^{v}+K^{va}C^{a}
...
.. math:: \\frac{\\partial C^{a}}{\\partial t}=-(\\nabla j_{p}-\\nabla j_{n})  -K^{va}C^{a}
...
def jgrad_central_onedim(Cprev, u, D, Jp_bnd, Jn_bnd, dx):
    ...
    # convert flux from (mmol/L/s) to (mmol/mm^2/s)
    jp = Jp * dx * 1e-3
    jn = Jn * dx * 1e-3
...
def onedim_onecomp_vel_diff(Lx, dx, tsamp, dtsim, u, D, Jp, Jn):
    ...
    Args:
        ...
        - D (numpy.ndarray float64):  
            - Pseudo diffusion (mm^2/s) at each voxel interface. Length = Nx + 1.  
...
def onedim_twocomp_vel_diff(Lx, dx, tsamp, dtsim, ua, uv, Kva, Da, Dv, Jpa, Jpv, Jna, Jnv):
    ...
    Args:
        ...
        - Da (numpy.ndarray float64):  
            - Arterial pseudo diffusion (mm^2/s) at each voxel interface. Length = Nx + 1.  
        - Dv (numpy.ndarray float64):  
            - Venous pseudo diffusion (mm^2/s) at each voxel interface. Length = Nx + 1.  
    ...
    from numpy import linspace, zeros
    from numpy import heaviside as H
    #from TKfunctions.forward_models.onedim.velocity import jgrad_fluxlimiter
    from TKfunctions.forward_models.onedim.velocity import onedim_twocomp_step
    # Caluclate new internal timesteps and timepoints
    T = npmax(tsamp)
    Ntsim = int(T/dtsim) + 1
    tsim = linspace(0, T, Ntsim)
...
def onedim_twocomp_vel_diff_slopelim(Lx, dx, tsamp, dtsim, ua, uv, Kva, Da, Dv, Cpa, Cpv, Cna, Cnv):
    ...
    from numpy import heaviside as H
    #from TKfunctions.forward_models.onedim.velocity import jgrad_fluxlimiter
    from TKfunctions.forward_models.onedim.velocity import onedim_twocomp_step_2ndorder
    # Caluclate new internal timesteps and timepoints
    T = npmax(tsamp)
    Ntsim = int(T/dtsim) + 1
    tsim = linspace(0, T, Ntsim)
