...
.. math:: v \\frac{\\partial c} {\\partial t}= -\\vec{\\nabla} \\cdot ( \\vec {f} c )+\\vec{\\nabla} \\cdot ( D \\vec{\\nabla} c )
...
.. math:: v^{a} \\frac{\\partial c^{a}}{\\partial t}=-\\vec{\\nabla} \\cdot ( \\vec{f}^{a} c^{a} )+\\vec{\\nabla} \\cdot ( D^{a} \\vec{\\nabla} c^{a} )-F^{a}

.. math:: v^{v} \\frac{\\partial c^{v}}{\\partial t}=-\\vec{\\nabla} \\cdot ( \\vec{f}^{v} c^{a} )+\\vec{\\nabla} \\cdot ( D^{v} \\vec{\\nabla} c^{v} )+Fc^{a}

With the measurement concentration being described as:

.. math:: C = v^{a}c^{a} + v^{v}c^{v}

This module casts these equations in terms of the positive 
direction flux and negative direction flux:

.. math:: v^{a}\\frac{\\partial c^{a}}{\\partial t}=-(\\nabla j_{p}-\\nabla j_{n})  -Fc^{a}
...
def jgrad_onedim(cprev, f, D, Jp_bnd, Jn_bnd, dx):
    ...
    - f (numpy.ndarray float64):  
        - Flow (ml/s/mm^2) at each interface.  
