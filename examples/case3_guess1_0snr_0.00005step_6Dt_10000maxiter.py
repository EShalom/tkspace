...
from TKfunctions.forward_models.onedim.cases import case_1D2C as gen_case
...
case        = 3 # case number for system type (detailed in testsystems.py)
SNR         = 0 # SNR for run
Dt          = 6  # sampling time (s)
maxiter     = 10000
updatemethod= 'adam'
step        = 0.00005
fdir        = 'case3_guess1_0snr_0.00005step_6Dt_10000maxiter'
mkdir(fdir)
#%%
...
fa1_guess = [12,12,5,8,3]
fv1_guess = [-12,-3,-10,-8,-12]
F_guess = [0.065,0.055,0.045,0.035,0.025]
va_frac_guess = [0.4,0.45,0.5,0.55,0.6]

fa1_init = fa1_guess[1-1]
fv1_init = fv1_guess[1-1]
F_init = np.ones(Nx)*F_guess[1-1]
Da_init=np.zeros(Nx+1)
Dv_init=np.zeros(Nx+1)
v=vguess
va_frac=np.ones(Nx)*va_frac_guess[1-1]

start_time = 12#len(tsamp)
...
lb = [-1,0,0,0.2,0.2,0]#(f, F, D, v, va_frac, J)
ub = [1,1,1,1,0.8,1]#(f, F, D, v, va_frac, J)
exit_change = [1e-4,1e-6,1e-9,1e-4,0]
max_vals = [14,0.07,4e-6,1,1,15] # fmax, Fmax, Dmax, vmax, va_frac_max, Jmax 
